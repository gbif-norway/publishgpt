...
class TestPythonRun(TestCase):
    ...
    def test_successful_execution_without_output(self):
        ...
        self.assertIn('executed successfully without errors', result)
...
class TestHandleTableBackups(TestCase):
    def setUp(self):
        ...
        self.table2 = Table.objects.create(dataset=self.dataset, title='Table 2', df=pd.DataFrame(['other data']))
    ...
    def test_deleted_tables(self):
        ...
        # Check that there's only 1 new table with an identical df to the old table, and with a deleted_at
        tables = Table.objects.exclude(id=self.table2.id)
    ...
    def test_handle_edits(self):
        ...
        # The edited table should have the updated dataframe
        edited_table = tables.filter(stale_at=None)[0]
        assert_frame_equal(edited_table.df, pd.DataFrame(['updated data']))

        # The backup should have the old dataframe, and a stale_at
        backup_table = tables.exclude(stale_at=None)[0]
        assert_frame_equal(backup_table.df, self.table1_args['df'])
        self.assertTrue(backup_table.stale_at != None)

        # Check the many-to-many link
        self.assertTrue(MessageTableAssociation.objects.filter(message=self.message, table=edited_table, operation=MessageTableAssociation.Operation.UPDATE).exists())
    ...
    def test_no_changes(self):
        ...
        self.assertEqual(list(tables), [self.table1, self.table2])
